...
public struct LMFeedPostDataModel {
    ...
    public var widgetAttachment: [LMFeedWidgetDataModel] = []
    ...
    public init(
        ...
        widgetAttachment: [LMFeedWidgetDataModel] = [],
        ...
        self.widgetAttachment = widgetAttachment
    }
}

extension LMFeedPostDataModel {
    init?(
        post: Post, users: [String: User],
        allTopics: [TopicFeedResponse.TopicResponse], widgets: [String: Widget],
        filteredComments: [String: Comment] = [:]
    ) {
        ...
        self.topics =
            post.topics?.compactMap { topicID in
                guard let topic = allTopics.first(where: { $0.id == topicID }),
                    let name = topic.name
                else { return nil }
                return .init(
                    topicName: name, topicID: topicID,
                    isEnabled: topic.isEnabled ?? false)
            } ?? []

        self.topResponse = Self.fetchTopResponse(
            for: post, users: users, filteredComments: filteredComments)
        ...
        let attachments = handleAttachments(
            for: postId, attachments: post.attachments ?? [], widgets: widgets,
            users: users)
        ...
        self.widgetAttachment = attachments.widgets

    }

    public func checkAspectRatios(attachments: [Attachment]) -> (
        allSameAspectRatio: Bool, aspectRatio: Double
    ) {
    ...
    for postID: String, attachments: [Attachment], widgets: [String:Widget],
        ...
        var tempWidgetAttachment: [LMFeedWidgetDataModel] = []
        ...
        attachments.forEach { attachment in
            if let type = attachment.attachmentType {
                ...
                case .doc:
                    if let url = attachment.attachmentMeta?.attachmentUrl {
                        let name =
                            attachment.attachmentMeta?.name
                            ?? "Unnamed Document"
                        tempDocumentAttachment.append(
                            .init(
                                url: url, name: name,
                ...
                case .link:
                    if let url = attachment.attachmentMeta?.ogTags?.url {
                        tempLinkAttachment = .init(
                            url: url,
                            title: attachment.attachmentMeta?.ogTags?.title,
                            description: attachment.attachmentMeta?.ogTags?
                                .description,
                            previewImage: attachment.attachmentMeta?.ogTags?
                                .image)
                    }
                case .poll:
        ...
        return (
            tempImageVideoAttachment, tempDocumentAttachment,
            tempLinkAttachment, poll, tempWidgetAttachment
        )
    }

    static func fetchTopResponse(
        for post: Post, users: [String: User],
        filteredComments: [String: Comment]
    ) -> LMFeedCommentDataModel? {
        guard let commentID = post.filteredComments?.first,
            let filteredComment = filteredComments[commentID],
            let user = users[filteredComment.uuid ?? ""]
        else { return nil }
...
extension LMFeedPostDataModel {
    public struct ImageVideoAttachment {
        ...
        public init(
            name: String, url: String, isVideo: Bool, size: Int, duration: Int?,
...
extension LMFeedPostDataModel {
    public struct DocumentAttachment {
        ...
        public init(
            url: String, name: String, format: String? = nil, size: Int? = nil,
            pageCount: Int? = nil
        ) {
            self.url = url
            self.name = name
...
extension LMFeedPostDataModel {
    public func getPostType() -> String {
        ...
        return self.imageVideoAttachment.first?.isVideo == true
            ? "video" : "image"
    }
}
